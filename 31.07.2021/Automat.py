from automatExeptions import NoGoodsForSelling
...
class Automat:
    ...
    def set_commands(self):          ## ustawia komendy dla kontrolek
    ...
    def put_number(self, number):           ## wywoływana po naciśnięciu przycisków numerycznych
        text = ""
        if len(self.code) in (0, 2):
            ...
            if not self.code in list(goods.keys()):
                ...
                text += "\n" 
                text += str(goods[self.code]["price"]/100)
                text += " złote"
